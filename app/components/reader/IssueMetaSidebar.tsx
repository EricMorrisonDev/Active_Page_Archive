...
export function IssueMetaSidebar({ issue }: { issue: Issue }) {
  return (
    <div className="p-6 pb-10">
      ...
      <h1 className="mt-6 font-serif text-2xl font-semibold tracking-tight text-neutral-900">
        {issue.title}
      </h1>
      ...
      {issue.summary ? (
        <p className="mt-4 font-sans text-sm leading-relaxed text-neutral-700">
          {issue.summary}
        </p>
      ) : null}
      {issue.tags.length > 0 ? (
        <div className="mt-6">
          ...
          <ul className="mt-2 flex flex-wrap gap-1.5">
            {issue.tags.map((slug) => (
              <li
                key={slug}
                className="rounded-sm border border-neutral-200 bg-white px-2 py-0.5 font-sans text-[11px] font-medium text-neutral-600 shadow-sm"
              >
                {tagLabel(slug)}
              </li>
            ))}
            ...
            {issue.featured_articles.map((article) => (
              <li key={`${article.page}-${article.title}`}>
                <Link
                  ...
                  className="font-sans text-sm text-neutral-800 underline-offset-2 hover:underline"
                >
